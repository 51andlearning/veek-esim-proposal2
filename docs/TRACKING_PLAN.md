...
- One form ships in v1: "Request a working session" (see `DEPLOYMENT.md`).
- Emit `form_submit` with `form_name = "working_session"` and a boolean `success`.
- Also log server-side in `/api/contact/route.ts`.
...
- Nonetheless, prefer cookieless analytics (Vercel Web Analytics) for v1 so no LGPD consent banner is required.
